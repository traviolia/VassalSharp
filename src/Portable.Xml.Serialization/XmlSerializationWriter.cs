...
";
    ...
    internal class XmlSerializationWriterCodeGen : XmlSerializationCodeGen
    {
        ...
        internal void GenerateBegin()
        {
            ...
            Writer.Write(" : ");
            Writer.Write(typeof(Portable.Xml.Serialization.XmlSerializationWriter).FullName);
            Writer.WriteLine(" {");
            Writer.Indent++;
            ...
            foreach (TypeScope scope in Scopes)
            {
                foreach (TypeMapping typeMapping in scope.TypeMappings)
                {
                        ...
                        !typeMapping.TypeDesc.IsRoot)
                    {
                        ...
                        Writer.Write(", new ");
                        Writer.Write(typeof(Portable.Xml.Serialization.XmlSerializationWriteCallback).FullName);
                        Writer.Write("(this.");
        ...
        private string GenerateMembersElement(XmlMembersMapping xmlMembersMapping)
        {
            ...
            if (hasWrapperElement)
            {
                ...
                if (xmlnsMember >= 0)
                {
                    ...
                    string source = "((" + typeof(Portable.Xml.Serialization.XmlSerializerNamespaces).FullName + ")p[" + xmlnsMember.ToString(CultureInfo.InvariantCulture) + "])";
