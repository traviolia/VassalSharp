...
namespace Portable.Xml.Serialization
{
    ...
    public class XmlReflectionImporter
    {
        ...
        internal static MethodInfo GetMethodFromSchemaProvider(XmlSchemaProviderAttribute provider, Type type)
        {
            ...
            if (!IsValidTypeNameOrIdentifier(provider.MethodName, false))
                ...
                if (attrs.Length > 0)
                {
                    ...
                    if (qname != null && !qname.IsEmpty)
                    {
                        ...
                        else
                        {
                            ...
                            if (xsdType != null)
                                //FIXME SetBase(serializableMapping, xsdType.DerivedFrom);
                                throw new NotImplementedException(); // see FIXME
        ...
        private void ImportAccessorMapping(MemberMapping accessor, FieldModel model, XmlAttributes a, string ns, Type choiceIdentifierType, bool rpc, bool openModel, RecursionLimiter limiter)
        {
            ...
            else if (!accessor.TypeDesc.IsVoid)
            {
                ...
                else if (a.Xmlns)
                {
                    ...
                    if (accessorType != typeof(Portable.Xml.Serialization.XmlSerializerNamespaces))
                    {
                        throw new InvalidOperationException(SR.Format(SR.XmlXmlnsInvalidType, accessorName, accessorType.FullName, typeof(Portable.Xml.Serialization.XmlSerializerNamespaces).FullName));
                    }
