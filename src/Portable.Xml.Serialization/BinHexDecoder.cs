...
namespace Portable.Xml.Serialization
{
    internal class BinHexDecoder : IncrementalReadDecoder
    {
        ...
        public static unsafe byte[] Decode(char[] chars, bool allowOddChars)
        {
            ...
            {
                throw new XmlException(SR.CreateMessage(SR.Xml_InvalidBinHexValueOddCount, new string(chars), 0, 0));
            }
                                    ...
                                    out int charsDecoded, out int bytesDecoded)
        {
            ...
            while (pChar < pCharsEndPos && pByte < pBytesEndPos)
            {
                ...
                {
                    throw new XmlException(SR.CreateMessage(SR.Xml_InvalidBinHexValue, new string(pChars, 0, (int)(pCharsEndPos - pChars)), 0, 0));
                }
