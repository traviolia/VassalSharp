...
namespace bsh
{
	...
	/// <summary>This class needs logic to prevent the right hand side of boolean logical
	/// expressions from being naively evaluated...  e.g. for "foo && bar" bar 
	/// should not be evaluated in the case where foo is true.
	/// </summary>
	[Serializable]
	class BSHTernaryExpression:SimpleNode
	{
		
		internal BSHTernaryExpression(int id):base(id)
		{
		}
