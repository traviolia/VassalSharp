...
namespace bsh
{
	...
	[Serializable]
	class BSHArguments:SimpleNode
	{
		internal BSHArguments(int id):base(id)
		{
		}
		
		/// <summary>This node holds a set of arguments for a method invocation or
		/// constructor call.
		...
		be compounded.
		*/
		public virtual System.Object[] getArguments(CallStack callstack, Interpreter interpreter)
		{
			// evaluate each child
			System.Object[] args = new System.Object[jjtGetNumChildren()];
			for (int i = 0; i < args.Length; i++)
			{
				args[i] = ((SimpleNode) jjtGetChild(i)).eval(callstack, interpreter);
				if (args[i] == Primitive.VOID)
					throw new EvalError("Undefined argument: " + ((SimpleNode) jjtGetChild(i)).Text, this, callstack);
			}
