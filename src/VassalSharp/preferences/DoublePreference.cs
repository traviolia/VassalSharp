...
namespace VassalSharp.preferences
{
	...
	/// <summary> A Decimal Number Module Preference.</summary>
	public class DoublePreference:BasicPreference
	{
		...
		{
			public AnonymousClassPropertyChangeListener(DoublePreference enclosingInstance)
			{
				InitBlock(enclosingInstance);
			}
			private void  InitBlock(DoublePreference enclosingInstance)
			{
				this.enclosingInstance = enclosingInstance;
			}
			private DoublePreference enclosingInstance;
			public DoublePreference Enclosing_Instance
			{
				get
				{
		...
		virtual public Configurer PreferenceConfigurer
		{
			get
			{
				if (config == null)
				{
					config = new DoubleConfigurer(getVariableName(), getDescription(), defaultValue);
		...
		protected internal double defaultValue = 0.0;
		new protected internal DoubleConfigurer config;
		...
		public virtual void  setDefaultValue(System.Object value_Renamed)
		{
			...
			defaultValue = ((System.Double) value_Renamed);
		}
	}
}